 ...
 */
import type { Facts, Checkin, PassUsage, Membership, Cohort } from '../domain/types';
import { buildSimulatedCohort, type SimulatedMember } from '../simulator/cohort';
import { HABITS, PHOTO_REFS, CHECKIN_TEXTS, MY_USER_ID, MY_MEMBERSHIP_ID } from './catalog';
 ...
 */
export function buildWorld(now: Date, mine: MyFacts, cohort: Cohort): World {
  const throughDay = Math.max(0, Math.min(getCohortDay(now, cohort), cohort.durationDays));
  ...
  const myMembership: Membership = {
    ...
    userId: MY_USER_ID,
    cohortId: cohort.id,
    joinedAt: `${cohort.startDate}T00:00:00+09:00`,
  };

  const facts: Facts = {
    habits: HABITS,
    cohorts: [cohort],
    memberships: [...sim.memberships, myMembership],
