...
export function getPassUsages(facts: Facts, membershipId: MembershipId): PassUsage[] {
...
/** 인증한 날 ∪ 면제권 쓴 날 */
export function getFilledDays(facts: Facts, membershipId: MembershipId): Set<CohortDay> {
  const days = new Set<CohortDay>();
  for (const c of getCheckins(facts, membershipId)) days.add(c.cohortDay);
  for (const p of getPassUsages(facts, membershipId)) days.add(p.cohortDay);
  return days;
}
...
export function getBestStreak(facts: Facts, membershipId: MembershipId, ctx: Ctx): number {
  ...
  let best = 0;
  let run = 0;
  for (let d = 1; d <= ctx.cohort.durationDays; d++) {
    if (filled.has(d)) {
      run++;
...
export function getProgress(facts: Facts, membershipId: MembershipId, ctx: Ctx): Progress {
  ...
  const checkins = getCheckins(facts, membershipId);
  const passes = getPassUsages(facts, membershipId);
  const filled = getFilledDays(facts, membershipId);
  ...
  return {
    day: getCohortDay(ctx.now, ctx.cohort),
...
export function getMemberState(facts: Facts, membershipId: MembershipId, ctx: Ctx): MemberState {
  ...
  if (getCheckins(facts, membershipId).length === 0) return 'day0';
  return 'ongoing';
}
